function best=getGenDisL_core(x,valid,part,depth,layer,gen,fset,pHW_avg,label_pHW_center,parent,theConf,getLossPara)
minGPUUnit=50000;
...
if(theConf.QA.IsUseGPU)
    ...
    best=getGenDisL_core_MEX(init,x,valid,part,depth,layer,gen,fset,pHW_avg,label_pHW_center,parent,xh,xw,weight,inValidX,minValue,theConf);
end
end


function best=getGenDisL_core_MEX(init,x,valid,part,depth,layer,gen,fset,pHW_avg,label_pHW_center,parent,xh,xw,weight,inValidX,minValue,theConf)
...
valid=double(valid);
fset=double(fset);
part.HoG=double(part.HoG);
[best.pHW_part,best.xHW,best.local,best.geo,best.app,best.gen_local,best.gen_geo,best.gen_SUM,best.parent,best.super_loc,best.reli,best.GenL,best.DisL,best.GenDisL]=getGenDisL_core_mex(init.xHW,x,valid,depth,part.DeltaHW,pHW_avg.scale,pHW_avg.center,label_pHW_center,[xh,xw],inValidX,minValue,fset,SearchStride,Stride,centerStart,gen.local,gen.geo,gen.SUM,parent.pHW,parent.valid,[weight.local,weight.geo,weight.app,weight.reli,weight.gen,weight.parent,weight.super_loc,weight.delta],part.HoG);
...
value.geo(list)=getLogGauss_([dh;dw],weight.delta/init.Stride);
idxList=HW2List_(value.xHW(:,list),xh);
value.app(list)=-(sum((part.HoG(:,list)-fset(:,idxList)).^2,1));
...
value.reli(list)=getLogGauss_(mu_reli,norm(weight.delta)); %%%%%%%%%%%%
...
if(~isempty(parent.valid))
    ...
    expGain=reshape(-getNorm(mu_parent)./getNorm(weight.delta./2),[parentNum,listlen]);
...
if(~isempty(label_pHW_center))
    ...
    value.super_loc(list)=getLogGauss_(mu_loc,weight.delta./2); %%%%%%%%%%%%
end
